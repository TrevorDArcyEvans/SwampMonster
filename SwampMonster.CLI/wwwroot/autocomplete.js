function autocomplete(inp, arr)
{
  ...
  inp.addEventListener("input", function (e)
  {
    ...
    for (i = 0; i < arr.length; i++)
    {
      ...
      {
        var match = arr[i].toUpperCase();
        var idxVal = match.indexOf(val.toUpperCase());

        /* create a DIV element for each matching element */
        b = document.createElement("DIV");

        /* make the matching letters bold */
        b.innerHTML = arr[i].substr(0, idxVal);
        b.innerHTML += "<strong>" + arr[i].substr(idxVal, val.length) + "</strong>";
        b.innerHTML += arr[i].substr(idxVal + val.length);
